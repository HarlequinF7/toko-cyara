...
<head>
	...
	$keyword = $_GET["id"];
	$kaa= $koneksi->query("SELECT * FROM kategori WHERE id_kategori='$keyword' ");
	$kaaa=$kaa->fetch_assoc();
	$setiapp = array();
	$jupuk=$koneksi->query("SELECT *, produk_foto.url as urll FROM produk JOIN produk_foto ON produk.id_produk=produk_foto.id_produk WHERE produk.id_kategori='$keyword' GROUP BY nama_produk");
...
<body class="js">
    ...
    <div class="container">
    	<div class="row">
			<h3 align="center" style="margin-bottom: 2px; border-bottom: 5px solid red; padding: 2px;">Kategori : <?php echo $kaaa['nama_kategori'] ?></h3><br>
    	</div><br>
		<?php if (empty($setiapp)): ?>
			<div class="alert alert-danger"><strong>Kategori Tersebut Kosong</strong></div><br>
			<?php endif ?>
		<div class="row">
						...
						<div class="col-xl-6 col-lg-6 col-md-6 col-6">
							<div class="product-img">
								<a href="<?php echo $setiap['slug'] ?>">
									...
									<?php else: ?>
									<img class="default-img" src="<?php echo $setiap['urll'] ?>" style="width: 100%; max-height: 170px" alt="#">
									<img class="hover-img" src="<?php echo $setiap['urll'] ?>" style="width: 100%; max-height: 170px"  alt="#">	
										
									<?php endif ?>
									
									<?php if ($kurangi1=='0'): ?>
									<?php elseif($jumlahkarakter>3): ?> 
		                            <span class="out-of-stock">Diskon Rp.<?php echo $kurangi1 ?></span>
		                            <?php else: ?>
		                            <span class="out-of-stock">Diskon <?php echo $kurangi1 ?>%</span>
									<?php endif ?>
								</a>
							</div>
						</div>							
						<div class="col-xl-6 col-lg-6 col-md-6 col-6">
							<div class="product-content" style="margin: 5px;" >
								...
								<div class="product-price" >
									...
									<?php else: ?>
									<span class="old" style="font-size: 22px;">Rp.<?php echo $price1 ?></span>
									<span style="font-size: 22px">Rp.<?php echo $totalkurang1 ?></span>
									<?php endif ?>
								</div>
							</div>
						</div>
